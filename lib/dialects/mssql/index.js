...
class Client_MSSQL extends Client {
  ...
  _query(connection, query) {
    return new Promise((resolve, reject) => {
      ...
      const request = this._makeRequest(query, (err, count) => {
        ...
        query.response = rows;
        query.resultSetSplits = resultSetSplits;
      ...
      request.on('doneInProc', (rowCount, more) => {
        debug('request::doneInProc');
        if (more && rowCount > 0 && rows.length > 0){
          resultSetSplits.push(rowCount);
        }
      });
  ...
  processResponse(query, runner) {
    ...
    let { response, resultSetSplits } = query;
    const { method } = query;
    ...
    if (resultSetSplits.length > 1) {
      response = resultSetSplits.map(count=>{
        return response.splice(0, count);
      })
    }
